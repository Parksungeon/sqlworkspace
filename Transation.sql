...
-- 이메일을 'email@email.com', 서버가 생성한 코드는 '0CMD'이다

-- 2. 이메일 인증
INSERT email_authentication VALUES ('email@email.com', '0CMD');

-- 서버가 지정한 이메일로 인증코드를 전송하고 사용자는 그 코드를 확인한 후 인증코드를 입력하여 인증함

-- 3. 이메일 인증 확인
SELECT * FROM email_authentication 
WHERE email = 'email@email.com' AND authentication_code = '0CMD';
...
-- 2. 이메일 인증
INSERT email_authentication VALUES ('email3@email.com', '0CMD');
...
-- 3. 이메일 인증 확인
SELECT * FROM email_authentication
WHERE email = 'email3@email.com' AND authentication_code = '0CMD';
...
-- LIMIT 레코드 개수, 개수 : 상위에서 제외 레코드 개수만큼 제외 후 개수만큼의 데이터만 보여줌
